...
- `U3_U4_TI_LM158_LM258_LM358_dual_op_amp_alt.pdf`
  ...
  - Confidence: probable family match. ST direct datasheet download timed out, so TI LM158/LM258/LM358-family datasheet is kept as alternate pinout/behavior reference. User probing now indicates U3 is powered from `ENC_VCC_COMMON`, likely pin 8 if the LM258 pinout matches.
...
- `U10_ST_LM193_LM293_LM393_dual_comparator.pdf`
  ...
  - Confidence: probable. ST datasheet lists SO-8 `LM293D/LM293DT` marking as `293`; pin 8 rings to `ENC_VCC_COMMON`; still confirm pin 4 GND and outputs pin 1 / pin 7 on the board.
...
- `U10`: likely ST `LM293` / `LM393` family dual comparator after user clarified top marking as `293`. Pin 8 is on `ENC_VCC_COMMON`; exact function in dock/power / encoder-sensor enable path is still TBD by continuity and voltage checks.
