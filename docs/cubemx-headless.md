...
### GPIO-EXTI: грань и NVIC headless НЕ ставятся (проверено, F071, 2026-06)
Пин-EXTI (`PXy.Signal=GPXTIn` + `GPIO_ModeDefaultEXTI`) CubeMX в `config load` принимает,
НО при загрузке хедлесс:
- **грань сбрасывает на `GPIO_MODE_IT_RISING`** — значения `IT_RISING_FALLING`/`IT_FALLING`
  из руками-собранного .ioc игнорируются (в generated-коде окажется `IT_RISING`);
- **`NVIC.EXTI2_3_IRQn`/`NVIC.EXTI4_15_IRQn=true\:...` молча выкидывает** → ни
  `HAL_NVIC_EnableIRQ`, ни `EXTIx_IRQHandler` не генерятся, прерывания EXTI мертвы.

Симптом «по-тихому»: бампер (idle-high, удар=спад) ловит RISING = отпускание; кастер/энкодеры
на оба фронта теряют половину разрешения; и вообще IRQ не включён.

**Решение проекта: всё EXTI генерит CubeMX, грань+NVIC ставятся в GUI, регенерим из GUI.**
- Грань (Rising/Falling/Both) — на пинах в Pinout (проверено: GUI-реген держит).
- NVIC — в табе System Core → NVIC включить галки **EXTI line 2 and 3** и **EXTI line 4 to 15**
  (приоритеты 2 и 1). Без них Cube не сгенерит ни enable, ни `EXTIx_IRQHandler` → прерывания
  мертвы. Галки держать ВКЛЮЧЁННЫМИ.
- В `USER CODE` руками EXTI НЕ держим (ни enable, ни хендлеры) — единственное пользовательское
  место это `HAL_GPIO_EXTI_Callback` (диспетчер в `main.c` USER CODE 4; Cube его не генерит).
- ⚠️ Регенерить **только из GUI**. Слепой headless `config load` сбрасывает грань на RISING и
  выкидывает `NVIC.EXTI*` — EXTI тихо ломается.
